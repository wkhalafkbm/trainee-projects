...
Trainees often default to making agents too autonomous because it feels more impressive. The evaluation should push back on this — a good agent knows its limits.
...
|---|---|
| **Skill design** | Each skill has a clear, distinct description; inputs and outputs are well-defined |
| **Guardrail placement** | Confirmation steps are in the right places — not so many that the agent is annoying, not so few that it acts recklessly |
| **Missing input handling** | Agent asks for what it needs rather than hallucinating values or silently failing |
| **Out-of-scope handling** | Agent declines gracefully and helpfully, not with a generic error |
| **Failure awareness** | Trainee can articulate *why* each failure mode occurs at the architectural level, not just describe the symptom |
