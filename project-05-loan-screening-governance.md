...
|---|---|
| **Explanation quality** | Explanations reference specific application data; they would be useful to a loan officer |
| **Bias finding** | Trainee identified a real disparity in the test data and investigated its cause, not just reported the numbers |
| **Mitigation attempt** | Mitigation was deliberate and the trainee assessed whether it worked — not just "I changed the prompt" |
| **Human review design** | Decline path is genuinely not autonomous; there is a clear hand-off to a human reviewer |
| **Governance report depth** | Limitations are acknowledged honestly; the trainee does not oversell what the system can do |

---

## Sample Application Dataset

Use these applications to build and test the pipeline. They include deliberately ambiguous cases and cases designed to surface potential bias.
