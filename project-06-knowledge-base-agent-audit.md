...
|---|---|
| **PII layer placement** | PII detection runs before the model call, not after — the model must never see raw PII |
| **Injection detection coverage** | Both pattern-match and secondary-classifier approaches are implemented; not just a blocklist of phrases |
| **Access control enforcement** | Role filtering happens in the retrieval query, not in post-processing; trainee can explain why this matters |
| **Audit log completeness** | Every field is populated for every interaction, including flagged ones; the hash chain is present |
| **Deployment readiness honesty** | Assessment identifies real gaps (e.g. no user authentication, no rate limiting, no formal security review) rather than claiming the system is production-ready |
